...
* BUG:当函数递归调用时，在函数体内无法看到自己的定义，因此没办法解析到本函数，等待解决。
* 解决办法：/目前通过在解析函数体之前向FuncSet注册只有函数名字的func对象，使得函数体解析过程中可以看到本函数的名字。
* 延申问题，如何处理函数相互调用问题，要么引入c/c++的定义，声明规则，要么采用多趟解析方法，首先生成全局的函数集合，然后进行解析。
* 倾向于第二种处理方法。
* BUG:上下文返回return_var_的时候，函数/方法上下文上面可能还有块上下文，导致返回nullptr。
